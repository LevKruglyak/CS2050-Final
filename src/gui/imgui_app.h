...
class HatcheryApp;
static HatcheryApp *app = nullptr;

struct AppSpecs {
  const char *name = "Hatchery App";
  uint32_t width = 200;
  uint32_t height = 200;
};
...
public:
  ...
  void create(AppSpecs specs) {
    ...
    window = glfwCreateWindow(specs.width, specs.height, specs.name, nullptr,
                              nullptr);
  ...
  void run() {
    ...
    ImVec4 clear_color = ImVec4(0.7f, 0.7f, 0.7f, 1.0f);
